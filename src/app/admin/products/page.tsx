  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total Products", value: totalProducts, icon: Package, color: "from-violet-500 to-purple-500", iconBg: "bg-violet-100 text-violet-600 dark:bg-violet-900/40 dark:text-violet-400" },
          { label: "Active", value: activeProducts, icon: CheckCircle2, color: "from-emerald-500 to-teal-500", iconBg: "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/40 dark:text-emerald-400" },
          { label: "Low Stock", value: lowStock, icon: AlertTriangle, color: "from-amber-500 to-orange-500", iconBg: "bg-amber-100 text-amber-600 dark:bg-amber-900/40 dark:text-amber-400" },
          { label: "Out of Stock", value: outOfStock, icon: XCircle, color: "from-red-500 to-rose-500", iconBg: "bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-400" },
        ].map((stat, i) => (
            ...
            <tbody>
              {(products || []).length > 0 ? (
                (products || []).map((product, idx) => {
                  ...
                  return (
                    <tr key={`admin-prod-${product.id || 'p'}-${idx}`} className="group border-b border-gray-50 dark:border-gray-800/60 animate-table-row" style={{ animationDelay: `${500 + idx * 60}ms` }}>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="font-semibold text-sm line-clamp-1 text-gray-900 dark:text-white group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors duration-300">{product.name}</p>
                      ...
                      <td className="py-4 px-6 text-right">
                        <AdminProductActions product={{ id: product.id, slug: product.slug, images: product.images }} />
                      </td>
                    </tr>
                  );
                })
